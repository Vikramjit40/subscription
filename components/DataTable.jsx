...
const SortingTable = props => {
  ...
  function toggles(item, index) {
    setModals(!modals); setDataModel(item); setIsOpen(false);
  ...
  function updcat(item) {
    if (document.getElementById("perup").value.length > 0 && document.getElementById("nexpayup").value.length > 0 && document.getElementById("upna").value.length > 0 && document.getElementById("cosup").value.length > 0 && document.getElementById("perup").value.length > 0) {
      for (let object of modata) {
        if (object.id === item.id) {
          ...
          object.next_payment = document.getElementById("nexpayup").value;
          object.period = document.getElementById("perup").value;
          object.cost = document.getElementById("cosup").value;
          object.color = document.getElementById("colup").value;
          if (document.getElementById("paymeup").value == "Credit card") {
            object.credit = "https://web.subly.app/payment-methods/card.svg"
          }
          else if (document.getElementById("paymeup").value == "Paypal") {
            object.credit = "https://web.subly.app/payment-methods/paypal.svg"
          }
          else {
            object.credit = ""
          }
    ...
    try {
      if (document.getElementById("pernew").value.length > 0 && document.getElementById("newna").value.length > 0 && document.getElementById("nexpaynew").value.length > 0 && document.getElementById("cosnew").value.length > 0) {
        ...
        obj["category"] = document.getElementById("categornew").value,
          ...
          obj["payment_method"] = document.getElementById("paymenew").value,
          obj["next_payment"] = document.getElementById("nexpaynew").value,
          obj["period"] = document.getElementById("pernew").value,
          obj["cost"] = document.getElementById("cosnew").value,
          obj["color"] = document.getElementById("colnew").value,
          obj["image"] = `https://${document.getElementById("urlnew").value}/favicon.ico`,
          obj["active"] = true,
          obj["id"] = modata[modata.length - 1].id + 1
        if (document.getElementById("paymenew").value == "Credit card") {
          obj["credit"] = "https://web.subly.app/payment-methods/card.svg"
        }
        else if (document.getElementById("paymenew").value == "Paypal") {
          obj["credit"] = "https://web.subly.app/payment-methods/paypal.svg"
        }
        else {
          obj["credit"] = ""
        };
    ...
    <>
          ...
          <div className=' flex gap-x-3 ml-12 newSub'>
            ...
            <Modal style={{ overflow: "hidden", borderRadius: "12px", border: "none", zIndex: "1" }} isOpen={modal2} fade={false} toggle={toggle2}  >
              ...
              <ModalBody style={{ backgroundColor: "#f7fafc" }}>
                ...
                <button onClick={() => { toggle4(); toggle2() }} style={{ backgroundColor: "rgba(0,0,0,0.80)", borderRadius: "8px", border: "none", color: "white", width: "100%", padding: "12px 0px", fontSize: "18px" }}>Add from scratch</button>
              </ModalBody>
            </Modal>
            <Modal scrollable={true} style={{ borderRadius: "12px", height: "600px", marginTop: "5rem" }} isOpen={modal4} fade={false} toggle={toggle4}  >
              <ModalHeader style={{ backgroundColor: "#f7fafc" }} toggle={toggle4}><h3 style={{ marginLeft: "165px", padding: "10px 0px" }}>Add Subscriptions</h3></ModalHeader>
              <ModalBody style={{ backgroundColor: "#f7fafc" }}>
                <h3>General</h3>
                <form id="add-form" style={{ padding: "15px 5px" }}>
                  <div style={{ display: "flex", fontSize: "17px", gap: "3rem" }}>
                    <span style={{ width: "50%" }}>
                      ...
                      <input required id="newna" className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", color: "black", backgroundColor: "#edf2f7", padding: "6px 0px 6px 10px", width: "100%" }} type='text'  ></input>
                    </span>
                    <span style={{ width: "50%" }}>
                      <label style={{ color: "rgba(0,0,0,0.64)" }}>Category</label><br /><br />
                      <select required className='hover:bg-[#e2e8f0]' style={{ border: "none", color: "black", borderRadius: "5px", backgroundColor: "#edf2f7", width: "100%", padding: "5.8px 0px 5.8px 10px", outline: "none" }} id="categornew" name="categorUpdat">
                  ...
                  <div style={{ display: "flex", fontSize: "17px", gap: "3rem", marginTop: "3rem" }}>
                    <span style={{ width: "50%" }}>
                      ...
                      <p style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", padding: "3px 0px 3px 10px" }}>https:// <input id="urlnew" className='hover:bg-[#e2e8f0]' style={{ border: "none", color: "black", backgroundColor: "#edf2f7", borderTopRightRadius: "5px", borderBottomRightRadius: "5px", padding: "6px 0px 6px 10px", width: "69%" }} type='text'></input>
                      </p>
                    </span>
                    <span style={{ width: "50%" }}>
                      <label style={{ color: "rgba(0,0,0,0.64)" }}>Project<span style={{ color: "red" }}> *</span></label><br /><br />
                      <select required id="projnew" className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", width: "100%", padding: "5.9px 0px 5.9px 10px", color: "black" }} >
                        <option value="Personal">Personal</option>
                      </select>
                    </span>
                  </div>
                  <div style={{ fontSize: "17px", marginTop: "3rem" }}>
                    <label style={{ color: "rgba(0,0,0,0.64)" }}>Color</label><br /><br />
                    <input id="colnew" type='color' style={{ width: "30px", cursor: "pointer" }} ></input>
                  </div>
                  <div style={{ display: "flex", fontSize: "17px", gap: "3rem", marginTop: "3rem" }}>
                    <span style={{ width: "50%" }}>
                    ...
                    <span style={{ width: "50%" }}>
                      ...
                      <select required id="pernew" className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", color: "black", width: "100%", padding: "5.9px 0px 5.9px 10px" }} >
                  ...
                  <div style={{ display: "flex", fontSize: "17px", gap: "3rem", marginTop: "3rem" }}>
                    <span style={{ width: "50%" }}>
                      <label style={{ color: "rgba(0,0,0,0.64)" }} >Next Payment<span style={{ color: "red" }}> *</span></label><br /><br />
                      <input required id="nexpaynew" className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", color: "black", padding: "6px 0px 6px 10px", width: "100%" }} type='date' ></input>
                    </span>
                    <span style={{ width: "50%" }}>
                      <label style={{ color: "rgba(0,0,0,0.64)" }}>Payment Method</label><br /><br />
                      <select id="paymenew" className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", color: "black", backgroundColor: "#edf2f7", width: "100%", padding: "5.9px 0px 5.9px 10px" }} >
            ...
            <div className='row'>
              <div className=' flex '>
                ...
                <select onChange={colorChange} id="fillters" className='font-bold cursor-pointer fil w-20 h-9 text-center ml-2 border border-slate-400 rounded-md focus:outline-0' style={{ color: "rgba(0,0,0,0.64)", backgroundColor: "#edf2f7" }}>
                  ...
                  <option style={{ backgroundColor: "white", color: "black" }} value="#e85454">Red</option>
                  <option style={{ backgroundColor: "white", color: "black" }} value="#5dbb63">Green</option>
                  <option style={{ backgroundColor: "white", color: "black" }} value="#6385ba">Blue</option>
                  <option style={{ backgroundColor: "white", color: "black" }} value="#3e8e8e">Lime</option>
                  <option style={{ backgroundColor: "white", color: "black" }} value="#b19de2">Purple</option>
                </select>
      ...
      `}</style>
        ...
        <tbody {...getTableBodyProps()}>
            ...
            <table id="myTable">
              <tbody>
                  ...
                  <tr key={index} style={{ borderTop: "2px solid #f7fafc", cursor: "pointer" }} id="fillters">
                    ...
                    <td> <button onClick={() => { toggling(index, item) }} className='w-8 h-8 focus:outline-none border-0 rounded-lg bg-[#e2e8f0]'>
                      ...
                      <div id={index} style={{ border: "1px solid #e2e8f0", borderRadius: "10px", boxShadow: "0 1px 2px 0 rgba(0,0,0,0.05)", position: "absolute", left: "63.5%", width: "16rem", display: "none", backgroundColor: "white", padding: "10px 2px" }} >
                        ...
                        <button className='hover:bg-[#e2e8f0] focus:outline-none' style={{ color: "inherit", width: "100%", border: "none", backgroundColor: "white", textAlign: "left", padding: "5px" }} onClick={() => { toggling(index); inactive(index, item) }} ><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" font-size="18" focusable="false" aria-hidden="true" class="chakra-menu__icon" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M7 11h10v2H7z"></path><path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"></path></svg> Move to <span id="acin">{active==true ? "Inactive" : "Active"}</span></button>
                ...
                {datamodal != undefined && (<Modal isOpen={modals} toggle={toggles} scrollable={true} style={{ height: "600px", marginTop: "5rem" }}  >
                  ...
                  <ModalBody style={{ backgroundColor: "#f7fafc", padding: "2px 25px 15px 25px" }}>
                    <Card style={{ backgroundColor: datamodal.color, borderRadius: "8px", width: "100%" }} >
                      <Card.Content >
                        ...
                        <Card.Description style={{ color: "white" }} ><p className='left floated ml-12'> {datamodal.category} </p><p className='right floated'>/ {datamodal.period}</p></Card.Description>
                      </Card.Content>
                    </Card>
                    <h3>General</h3>
                    <form id="update-form">
                      <div style={{ display: "flex", fontSize: "17px", gap: "3rem" }}>
                        <span style={{ width: "50%" }}>
                          <label style={{ color: "rgba(0,0,0,0.64)" }} >Name<span style={{ color: "red" }}> *</span></label><br /><br />
                          <input required id="upna" className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", color: "black", backgroundColor: "#edf2f7", padding: "6px 0px 6px 10px", width: "100%" }} type='text' defaultValue={datamodal.first_name} ></input>
                        </span>
                        <span style={{ width: "50%" }}>
                          <label style={{ color: "rgba(0,0,0,0.64)" }}>Category</label><br /><br />
                          <select className='hover:bg-[#e2e8f0]' style={{ border: "none", color: "black", borderRadius: "5px", backgroundColor: "#edf2f7", width: "100%", padding: "5.8px 0px 5.8px 10px", outline: "none" }} defaultValue={selecVal} id="categorUpdat" name="categorUpdat">
                            <option value="-">Select Category</option>
                            <option value="Recruiting" >Recruiting</option>
                            <option value="Hosting" >Hosting</option>
                            <option value="Accounting" >Accounting</option>
                      ...
                      <div style={{ display: "flex", fontSize: "17px", gap: "3rem", marginTop: "3rem" }}>
                        <span style={{ width: "50%" }}>
                          ...
                          <p style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", padding: "3px 0px 3px 10px" }}>https:// <input id="urlup" className='hover:bg-[#e2e8f0]' style={{ border: "none", color: "black", backgroundColor: "#edf2f7", borderTopRightRadius: "5px", borderBottomRightRadius: "5px", padding: "6px 0px 6px 10px", width: "69%" }} type='text' defaultValue={datamodal.url}></input>
                          </p>
                        </span>
                        <span style={{ width: "50%" }}>
                          <label style={{ color: "rgba(0,0,0,0.64)" }}>Project<span style={{ color: "red" }}> *</span></label><br /><br />
                          <select id="projup" className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", width: "100%", padding: "5.9px 0px 5.9px 10px", color: "black" }} >
                            <option value="Personal">Personal</option>
                          </select>
                        </span>
                      </div>
                      <div style={{ fontSize: "17px", marginTop: "3rem" }}>
                        <label style={{ color: "rgba(0,0,0,0.64)" }}>Color</label><br /><br />
                        <input id="colup" type='color' style={{ width: "30px", cursor: "pointer" }} defaultValue={datamodal.color}></input>
                      </div>
                      <div style={{ display: "flex", fontSize: "17px", gap: "3rem", marginTop: "3rem" }}>
                        <span style={{ width: "50%" }}>
                          <label style={{ color: "rgba(0,0,0,0.64)" }} >Cost<span style={{ color: "red" }}> *</span></label><br /><br />
                        ...
                        <span style={{ width: "50%" }}>
                          <label style={{ color: "rgba(0,0,0,0.64)" }}>Expense Type<span style={{ color: "red" }}> *</span></label><br /><br />
                          <select id="perup" required className='hover:bg-[#e2e8f0]' defaultValue={datamodal.period} style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", color: "black", width: "100%", padding: "5.9px 0px 5.9px 10px" }} >
                            <option value="">Choose Expense Type</option>
                            <option value="One time">One Time</option>
                            <option value="1 month">1 month</option>
                            <option value="2 month">2 months</option>
                      ...
                      <div style={{ display: "flex", fontSize: "17px", gap: "3rem", marginTop: "3rem" }}>
                        <span style={{ width: "50%" }}>
                          <label style={{ color: "rgba(0,0,0,0.64)" }} >Next Payment<span style={{ color: "red" }}> *</span></label><br /><br />
                          <input id="nexpayup" required className='hover:bg-[#e2e8f0]' style={{ border: "none", borderRadius: "5px", backgroundColor: "#edf2f7", color: "black", padding: "6px 0px 6px 10px", width: "100%" }} type='date' defaultValue={subname}></input>
                        </span>
                        <span style={{ width: "50%" }}>
                          <label style={{ color: "rgba(0,0,0,0.64)" }}>Payment Method</label><br /><br />
                          <select id="paymeup" className='hover:bg-[#e2e8f0]' defaultValue={datamodal.payment_method} style={{ border: "none", borderRadius: "5px", color: "black", backgroundColor: "#edf2f7", width: "100%", padding: "5.9px 0px 5.9px 10px" }} >
                            <option value="-">Choose Payment Method</option>
                            <option value="Credit card">Credit card</option>
                            <option value="Paypal">Paypal</option>
                          </select>
                  ...
                  <ModalFooter>
                    <button form='update-form' type='submit' onClick={() => { updcat(datamodal) }} className='hover:bg-[black]' style={{ width: "100%", color: "white", backgroundColor: "rgba(0,0,0,0.8)", padding: "1rem", fontWeight: "bold", fontSize: "20px", borderRadius: "10px", border: "none" }}>Update Subscription</button>
                  </ModalFooter>
                </Modal>)}
              </tbody>
            </table>
          ...
          <Card.Group className='table-container mt-2 ml-1' id="blockv" style={{ padding: "0.1rem", display: "none", height: data.length != 0 ? "560px" : "40px", overflow: "scroll", overflowX: "hidden" }} >
            <Grid columns={3}
              stackable > {
