...
const newItems=[
    {date:"29 May, 2023",heading:"Direct File Uploads",description:"Now you can directly upload files to subscriptions. Enhance your subscription and recurring expense management by attaching relevant files directly to your subscriptions."},
    {date:"28 Mar, 2023",heading:"Say Hello to Our New and Improved Notes!",description:"Experience an upgraded note-taking game on Subly. Format notes related to your subscriptions and recurring expenses with ease and flexibility using the enhanced block-styled notes engine."},
    {date:"15 Mar, 2023",heading:"Two-factor authentication (2FA)",description:"With 2FA, you can now add an extra layer of security to your account by requiring a time-based one-time password (TOTP) generated by your authentication app."},
    {date:"29 Jan, 2023",heading:"Introducing Webhooks",description:"Webhooks allows you to connect Subly to other applications and automate tasks based on events within Subly"},
    {date:"1 Jan, 2023",heading:"Discord integration",description:"Connect your Discord server to Subly to receive payment reminders and never miss a payment."},
    {date:"21 Dec, 2023",heading:"Search & custom icons",description:`New features make it easier to manage subscriptions with quick search, custom icons, and "add to calendar" button for payment dates.`},
    {date:"4 Dec, 2023",heading:"Slack integration",description:"We are excited to announce a new integration with Slack! Now, users on paid plans will be able to receive payment reminders on their preferred Slack channel."},
    {date:"28 Nov, 2023",heading:"Introducing: Team members and project budgets",description:"Invite team members to join your Subly workspace"},
    {date:"1 Nov, 2023",heading:"Telegram integration & calendar view",description:"Introducing our telegram integration and the new calendar view to visualize subscriptions"},
...
                            <label style={{fontSize:"24px"}} for="veryhappy">😍</label>
        ...
        <div onClick={toggle2} id="what" data-placement="right" className='flex items-center w-full h-full hover:bg-hoverCol mb-8 mt-2'><div><img src="/exclamation.svg"  className=' p-3' /></div>{toggleCollapse && (
                ...
                <Modal isOpen={modal2} toggle={toggle2} fade={true} style={{padding:"0px 8px",height:"100%", top: "-3%", left: "51%", transform: "translateY(-3%)", transform: "translatex(-51%)"}}>
                <ModalHeader style={{backgroundColor:"#f7fafc"}} toggle={toggle2}><h3>What's New</h3></ModalHeader>
                <ModalBody style={{backgroundColor:"#f7fafc"}}>
                    <div>
                    {newItems.map(newItem => (
                        <div onMouseOver={() => { document.getElementById(newItem.heading).style.color="#006ad3"}} onMouseOut={() => { document.getElementById(newItem.heading).style.color="black"}} style={{boxShadow:"0 4px 6px -1px rgba(0, 0, 0, 0.1),0 2px 4px -1px rgba(0, 0, 0, 0.06)",cursor:"pointer",display:"flex",flexDirection:"column",gap:"0.5rem",padding:"1.5rem 1rem",margin:"4px 0px"}}>
                            <p style={{color:"rgba(0,0,0,0.48)",fontSize:"16px",marginBottom:"2px"}}>{newItem.date}</p>
                            <h3 id={newItem.heading} style={{margin:"-2px 0px",fontSize:"20px"}}>{newItem.heading}</h3>
                            <p style={{fontSize:"16px",marginBottom:"2px"}}>{newItem.description}</p>
                        </div>
                    ))}
                    </div>
                </ModalBody>
